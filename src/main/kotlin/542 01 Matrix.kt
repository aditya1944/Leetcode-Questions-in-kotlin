    ...
    for(rowIndex in mat.indices){
        for(colIndex in mat[rowIndex].indices){
            ...
            if(mat[rowIndex][colIndex]==1){
                ...
                if(colIndex-1>=0){
                    distanceMatrix[rowIndex][colIndex] = kotlin.math.min(distanceMatrix[rowIndex][colIndex-1]+1, distanceMatrix[rowIndex][colIndex]);
                }
            }else{
        ...
        for(colIndex in mat[rowIndex].size-1 downTo 0){
            ...
            if(mat[rowIndex][colIndex]==1){
                if(rowIndex+1<mat.size){
                    distanceMatrix[rowIndex][colIndex] = kotlin.math.min(distanceMatrix[rowIndex+1][colIndex]+1, distanceMatrix[rowIndex][colIndex]);
                }
